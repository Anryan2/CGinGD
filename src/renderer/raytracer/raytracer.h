...
namespace cg::renderer
{
	...
	{	
		...
		for(int fraim_id = 0; fraim_id < accumulation_num; fraim_id++){
			...
			const triangle<VB>& triangle, const ray& ray) const
	{
		...
		float inf_det = 1.f/det;
		float3 tvec = ray.position - triangle.a;
		float u = dot(tvec, pvec) * inf_det;
		if (u < 0.f || u > 1.f){
			return payload;
		}

		float3 gvec = cross(tvec, triangle.ba);
		float v = dot(ray.direction, gvec) * inf_det;
		if (v < 0.f || u + v > 1.f ){
			return payload;
		}

		payload.t = dot(triangle.ca, gvec)*inf_det;
